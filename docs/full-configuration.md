...
- **"httpd"**, the configuration for the HTTP server used to serve REST API and to expose the built-in web interface
  - `bindings`, list of structs. Each struct has the following fields:
    ...
    - `share_expiration_days`, integer. Defines a default amount of days for a newly created share to expire if a user did not pick an expiration date themself. Leave at 0 to disable. Default: 0.
