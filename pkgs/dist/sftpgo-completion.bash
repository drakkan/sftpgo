...
__sftpgo_get_completion_results() {
    ...
    # Calling ${words[0]} instead of directly sftpgo allows handling aliases
    args=("${words[@]:1}")
        ...
        if (((directive & shellCompDirectiveKeepOrder) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                ...
                if [[ ${BASH_VERSINFO[0]} -lt 4 || ( ${BASH_VERSINFO[0]} -eq 4 && ${BASH_VERSINFO[1]} -lt 4 ) ]]; then
                    __sftpgo_debug "No sort directive not supported in this version of bash"
                else
                    __sftpgo_debug "Activating keep order"
                    compopt -o nosort
                fi
            else
                __sftpgo_debug "No sort directive not supported in this version of bash"
...
__sftpgo_extract_activeHelp() {
    ...
    while IFS='' read -r comp; do
        ...
        if [[ ${comp:0:endIndex} == $activeHelpMarker ]]; then
            comp=${comp:endIndex}
            __sftpgo_debug "ActiveHelp found: $comp"
            if [[ -n $comp ]]; then
                activeHelp+=("$comp")
            fi
        else
...
__sftpgo_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        ...
        __sftpgo_debug "Removed description from single completion, which is now: ${COMPREPLY[0]}"
    else
        # Format the descriptions
        __sftpgo_format_comp_descriptions $longest
    fi
}
